...
export default function App() {
  ...
  const [isDeleted, setIsDeleted] = useState(
    Array.from({ length: galleryImages.length }, () => false)
  );

  const deletedImageLength = isDeleted.filter((data) => data !== false).length;
  const displayedImageLength = 8 - deletedImageLength;
  ...
  const handleRestore = (e: React.MouseEvent, index: number) => {
    e.preventDefault();
    setIsDeleted((prev) => {
      const newDeleted = [...prev];
  ...
  const handleRestoreAll = (e: React.MouseEvent) => {
    e.preventDefault();
    setIsDeleted([false, false, false, false, false, false, false, false]);
  };
          ...
          <div className="flex justify-between items-start max-w-7xl mx-auto mb-8">
            <div className="flex gap-3">
              ...
              >
                ...
                Restore All {deletedImageLength}
              </button>
            </div>
          </div>
        )}
              ...
              <section className="relative overflow-hidden h-full">
                ...
                {isDeleted[index] && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                    <button
                      onClick={(e) => handleRestore(e, index)}
      ...
      <footer className="text-center py-6 sm:py-8 px-4 sm:px-6 border-t border-border bg-muted/30">
        <p className="text-muted-foreground text-sm sm:text-base">
          ...
          <span className="block mt-2 text-xs sm:text-sm">
            {deletedImageLength} images hidden • {displayedImageLength}{" "}
            displayed
          </span>
        </p>
      </footer>
